...
+(UIImage*)sldsIcon:(NSInteger)iconType withColor:(UIColor*)iconColor andBGColor:(UIColor*)bgColor andSize:(short)size{
    ...
    CGFloat const *fgCcomps = CGColorGetComponents(iconColor.CGColor);
    CGFloat const *bgCcomps = CGColorGetComponents(bgColor.CGColor);
    NSString *iconKey = [NSString stringWithFormat:@"%ld%f%f%f%f%f%f%f%f%d", (long)iconType,
        fgCcomps[0], fgCcomps[1], fgCcomps[2], fgCcomps[3], bgCcomps[0],
        bgCcomps[1], bgCcomps[2], bgCcomps[3], size];
